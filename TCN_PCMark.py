...
batch_size, time_steps, input_dim = None,29,1

def get_x_y():

    # 读取数据
    input_data_x = pd.read_excel(r'./PCMark.xlsx', sheet_name='Sheet1', header=None)
    input_data_y = pd.read_excel(r'./PCMark_energy.xlsx', sheet_name='Sheet1', header=None)
    ...
    input_data_x = input_data_x.reshape(3120, 29, 1)
...
tcn_full_summary(m, expand_residual_blocks=False)
m.fit(train_x, train_y, epochs=100, validation_split=0.2)

y_predict = m.predict(test_x)
y_predict = list(y_predict)

df_data1 = pd.DataFrame(y_predict)
outputfile1 = "Predict_PCMark_energy.xlsx"
df_data1.to_excel(outputfile1)

df_data2 = pd.DataFrame(test_y)
outputfile2 = "True_PCmark_energy.xlsx"
